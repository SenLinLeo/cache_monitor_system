...
func (e *Exporter) scrapeRedisHost(scrapes chan<- scrapeResult, addr string, idx int) error {
	...
	e.extractCodisMetrics(addr, e.redis.Aliases[idx])
	...
	var slowlog Slog
	if values, err := redis.Values(c.Do("SLOWLOG", "GET")); err == nil {
		for _, v := range values {
			...
			case []interface{}:
				for i, u := range value {
					...
					if i == 0 {
						slowlog.Uuid = u.(int64)
					}
					if i == 1 {
						slowlog.ExecuteTime = time.Unix(u.(int64), 0).Format("2006-01-02 15:04:05")
					}
					if i == 2 {
						slowlog.UsedTime = strconv.FormatInt(u.(int64), 10)
					}
					fmt.Println("testv:", i+1, u)
				}

				title := fmt.Sprintf("%s\n", v)
				strLen := len(title)
				fmt.Println(strLen)
				slowlog.Args = title[54 : strLen-3]
				fmt.Println(j+1, title[54:strLen-3], "\n")

				e.metricsMtx.RLock()
				e.metrics["slow_log_list"].WithLabelValues(addr, title[54:strLen-3], slowlog.UsedTime, slowlog.ExecuteTime ).Set(1)
				e.metricsMtx.RUnlock()

				// Slowlogs.WithLabelValues("173.30.60.128", title[54:strLen-3], slowlog.UsedTime, slowlog.ExecuteTime ).Set(1)
			default:
